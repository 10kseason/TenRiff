TenRiff source package notes (`1.4.5.1`)
...
- The current source line is `1.4.5.1`; it keeps P64 and the host beam safety solver authoritative and enables the generalized NK3 pattern MLP only for non-10K sources converted to 10K.
...
- NK3 requires OpenVINO. Windows release packages bundle the OpenVINO 2026.2.1 GPU/CPU runtime, available NPU plugin/compiler components, and original license notices; source builders can provide `OpenVINO_DIR` or disable `TENRIFF_ENABLE_NK3_ONNX`. P64 defaults to strict GPU and accepts `TENRIFF_NK3_DEVICE=CPU`; only non-10K to 10K enables the MLP, which verifies and tries NPU, GPU, then CPU.
- Official 1.4.5.1 builds and Windows archives do not build or ship the standalone BMS key-converter CLI/GUI. Its source remains development-only and the top-level CMake option defaults to `TENRIFF_BUILD_STANDALONE_BMS_KEY_CONVERTER=OFF`.
