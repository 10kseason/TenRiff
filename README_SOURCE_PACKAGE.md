TenRiff source package notes (`1.2.93 stable`)
...
- The current source line is `1.2.93 stable`; it supports BMS-family charts, 4K through 14K key modes, and native/LR2 skins, keeps MPG/MPEG video BGA decoding with an FFmpeg fallback, and exposes an External ONNX Upscaler that remains off until the user enables it and acknowledges the high-spec warning.
