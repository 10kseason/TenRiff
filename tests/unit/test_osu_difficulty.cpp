...
using tenriff::chart::calculate_osu_10k_difficulty;

namespace {

OsuManiaChart make_chart() {
    OsuManiaChart chart;
    chart.key_count = 10;
...
TEST_CASE("10k difficulty rates dense streams above sparse streams") {
    ...
    const OsuDifficultyMetrics sparse_metrics = calculate_osu_10k_difficulty(sparse);
    const OsuDifficultyMetrics dense_metrics = calculate_osu_10k_difficulty(dense);
...
TEST_CASE("10k difficulty keeps jack-heavy charts above split patterns") {
    ...
    const OsuDifficultyMetrics jack_metrics = calculate_osu_10k_difficulty(jack);
    const OsuDifficultyMetrics split_metrics = calculate_osu_10k_difficulty(split);
