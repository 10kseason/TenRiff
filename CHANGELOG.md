...
- 난이도표 선택 창에 `F4 기본 LV / Native LV`를 추가합니다. 외부 표를 해제하고 원래 LV 표시·정렬·그룹·필터로 돌아가며, 정상 곡 캐시가 있으면 전체 재스캔 없이 적용합니다.
- 일반 싱글플레이 재개에 3·2·1 카운트다운을 추가하고 세션 믹스의 로딩·카운트다운·플레이에서 Esc를 무시합니다.
- 곡의 대표 BPM은 실제 진행 누적 시간이 가장 긴 BPM을 사용합니다. STOP 대기는 제외하고 원래 시작 BPM은 타이밍과 기존 리플레이/키 변환용으로 보존합니다.
- 스킨 설정과 필요한 이미지를 `.trskin` 파일로 내보내고 다른 PC에서 가져와 적용할 수 있습니다.
- 한글 경로의 NK3 모델·프로필·리플레이 검증기 경로 처리를 수정하고, 상대 이탈 후에도 해당 라운드의 멀티 결과를 유지합니다.
- 단위인정/코스 게이지를 LR2 beta3 역공학 참조 규칙에 맞추고 간접미스를 유지합니다. 일반 Easy/Normal/Hard 밸런스는 그대로 둡니다.
- 기본 난이도표 선택에 5키 에리, 7키 에리, 10키 리바이브를 추가합니다.
- 소스 관리에 폴더 추가와 Del 목록 제거를 추가합니다. 실제 곡 파일은 삭제하지 않습니다.

검증 결과와 ASIO 장치 확인 범위는 [1.7.2 릴리스 게이트](docs/release-1.7.2-gate.md)에 기록합니다. 이전 로컬 1.7.1 검사 횟수는 이번 릴리스의 검증 결과로 재사용하지 않습니다.
...
- 멀티플레이 인게임·결과가 최대 8인 참가자 전원을 표시합니다. 미도착 점수는 대기 상태로 유지하며 동점은 같은 순위를 사용합니다.
- 점수 비교 막대는 현재 점수 비율을 사용하고 상대 목록 위에 배치합니다. 실패 후 관전은 모든 상대의 종료 상태를 확인합니다.
- P-GREAT는 노란 글자, 무지개 반짝임과 튀는 모션으로 표시합니다. GREAT는 기존 청록색, GOOD은 회색이며 하위 판정 글자는 움직이지 않습니다.
- FAST/SLOW는 현재 판정에 해당하는 한쪽만 표시합니다. 판정과 콤보의 X/Y 위치를 각각 조절하고 저장할 수 있습니다.
- 키빔의 200ms 감쇠를 렌더 프레임 사이에서도 이어서 계산합니다. 60/144Hz 표현을 지원하며 실제 FPS 보장은 아닙니다.
- 인게임 전체 믹스에 적용하는 오디오 노멀라이즈 ON/OFF를 추가했습니다. 기본값은 OFF입니다.

### Menus
- 옵션은 고유한 파스텔 색의 5×2 카드로 구성하며, 모드 설정과 키 입력 테스트 바로가기를 추가했습니다.
- 선곡 목록의 긴 LEVEL 표기를 정리하고, 검색·정렬 옆에 난이도표 이름과 URL/파일/기본 LV 컨트롤을 배치했습니다.
- 난이도표 URL은 선곡 화면에서 편집하며, 잘못된 주소는 현재 표를 유지한 채 오류를 표시합니다.

### Verification and Known Reports
- 기존 판정·점수 산식, 리플레이 형식, 네트워크 프로토콜과 서버 코드는 변경하지 않았습니다.
- 검증 절차와 수동 확인 범위는 `docs/release-1.7.1-gate.md`에 기록합니다.
- 1.7.0의 롱노트 릴리즈 중복 소리 제보는 미재현 상태입니다. 실제 입력→키음→믹서 경로의 회귀 검사를 추가했으며, 수정 완료로 주장하지 않습니다.

## [1.7.0] - 2026-09-05

### UI Update

- 기본 메인·선곡·결과·공통 설정 화면의 색상, 패널, 글꼴 크기와 여백을 통일했습니다. 네온 테두리와 배경 장식을 줄여 곡과 기록, 주요 동작이 잘 보이도록 했습니다.
- 메인 메뉴는 플레이/곡 폴더 추가를 강조하고 멀티플레이·옵션·종료에 짧은 설명을 표시합니다. 기존 키보드 선택과 진입 동작을 유지합니다.
- 결과 화면은 큰 점수와 등급·정확도 중심으로 재배치하고 판정 숫자의 가독성과 버튼 정렬을 개선했습니다. 타이밍 그래프는 기존 평균·표준편차 기반 추정임을 명확히 표시합니다.
- 곡 필터·세션 믹스·모드·키맵·프로필 등 공통 화면에서 설정 목록과 줄바꿈되는 페이지형 안내를 분리했습니다. 안내 페이지 이동은 설정값과 선택 행을 바꾸지 않습니다.
- 스킨 설정의 실시간 미리보기를 유지하고 밝은 색상 견본 위 숫자의 대비를 개선했습니다. 사용자 TenRiff 타이틀/로비 스킨은 기존 레이아웃을 유지합니다.

### Fixed

- 선곡 상단 Session Mix 탭 잘림과 검색·뒤로·결과 버튼의 수직 정렬을 수정했습니다.
- 빈 선곡 목록의 안내문을 줄바꿈하고, 선택한 곡이 없을 때 표시되던 Selected 배지와 임의 색상 이미지를 제거했습니다.
- 공통 설정에서 성능 오버레이가 표시되면 안내 패널을 아래로 배치해 겹침을 피합니다.

### Release

- Windows x64 실행 패키지, 소스 패키지와 SHA-256 목록을 제공합니다.
- 판정·점수·리플레이·온라인 기록 프로토콜을 바꾸지 않는 UI 릴리스입니다. 실제 타이밍 분포 등 결과 분석 기능 확장은 이후 작업입니다.
- 검증 범위와 제한은 `docs/release-1.7.0-gate.md`에 기록합니다.
...
- 메인 API가 메인 IP 전용 TLS 인증서를 사용하도록 배포 구성을 수정했습니다.
- 메인 API에 한해 서버 leaf 인증서의 SHA-256을 직접 검증해, 시스템 인증서 저장소 변경 없이 자체 서명 인증서를 안전하게 사용합니다. 사설 API는 기존 Windows TLS 검증을 그대로 사용합니다.

### Release

- stable baseline과 앱 버전은 `1.5.1`을 유지하며, 수정 자산은 `1.5.1-hotfix.4` 태그로 별도 배포합니다.
