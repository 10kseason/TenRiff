...
- `backend` (string)
  - `polling | rawinput`
  - 現行 `1.4.4` リリースラインの既定値は `rawinput`
...
- `key_conversion_algorithm` (string)
  ...
  - 既定値は `krrcream`。NK3 は同じ key count でも remaster を実行し、既定で strict OpenVINO GPU を使用
  ...
  - NK3 は P64 hybrid ONNX と host beam safety solver を組み合わせ、`TENRIFF_NK3_DEVICE=CPU` で strict CPU を選択可能。NPU と自動 fallback は非対応
