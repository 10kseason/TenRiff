...
- `backend` (string)
  - `polling | rawinput`
  - defaults to `rawinput` on the current `1.4.5.2` release line
...
- `judgement_hz` (int)
  ...
  - the current `1.4.5.2` runtime no longer drives a separate audio-thread judgement sub-step loop from this value
