...
- Judge:
  ...
  - 게임 시작 직후 `ClockSync`가 아직 추정치를 만들기 전에는 startup anchor(`playback_sample + callback time`) 기준으로 입력 샘플을 선형 환산해, 첫 몇 노트 입력이 현재 playback sample 한 점으로 뭉개지지 않도록 함
...
- 새 사용자 프로필은 자동 생성
- 마지막으로 스테이징된 배포 패키지는 `Baepoks/TenRiff-1.0.3`
...
- 마지막으로 스테이징된 공개 소스 패키지는 `opensource-Tenriff-source/TenRiff-1.0.3-source`처럼 버전별로 별도 스테이징
...
- `cmake -S opensource-Tenriff-source/TenRiff-1.0.3-source -B opensource-Tenriff-source/TenRiff-1.0.3-source/build-check -G "Visual Studio 17 2022" -A x64`
- `cmake --build opensource-Tenriff-source/TenRiff-1.0.3-source/build-check --config Release --target bms_parser_tests`
- `opensource-Tenriff-source/TenRiff-1.0.3-source/build-check/Release/bms_parser_tests.exe`
