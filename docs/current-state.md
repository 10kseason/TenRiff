...
- `1.2.0 stable`의 gameplay live 입력은 저장된 RawInput을 우선하면서 같은 `InputThread`의 bound-key polling shadow를 항상 유지하고, 시작 실패뿐 아니라 message pump의 예기치 않은 종료도 queue/state reset 없이 현재 producer thread에서 Polling으로 전환함
...
- Graphics:
  ...
  - `visual_offset_ms`
  - `performance_overlay`
  - `background_upscale_mode=lunasr|off`: 저해상도 BMS 이미지 BGA 및 osu!mania 배경을 Windows ML worker에서 FHD로 비동기 보간하고 실패 시 native bitmap 유지
...
- 새 사용자 프로필은 자동 생성
- 현재 정식 P2P 배포 라인은 `TenRiff 1.2.0 stable`
