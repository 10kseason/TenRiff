...
- `1.2.3`은 LunaSR 고정 RGB x2 성능 게이트를 200 FPS에서 35 FPS로 낮추고, staged32 런타임을 35/35 Conv가 양자화된 static S8S8 QDQ INT8 모델로 전환.
...
- Graphics:
  ...
  - `visual_offset_ms`
  - `performance_overlay`
  - `background_upscale_mode=lunasr|off`: staged32 RGB INT8 QDQ WinML 추론 벤치마크가 35 FPS 이상일 때만 이미지·동영상 BGA를 FHD x2 처리하며, MPG/MPEG는 Media Foundation 실패 시 FFmpeg 폴백 사용
