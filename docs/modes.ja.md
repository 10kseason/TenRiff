...
- `key_conversion_algorithm`: `krrcream | nk2 | nk3`（既定値は `krrcream`。Krrcream は元 note の再配置のみ、nK2 は key count 拡張中の安全な support note 生成、NK3 は同梱 P64 hybrid ONNX と host beam32 による再配置・support-note candidate 評価を行う）
