...
- `key_conversion_algorithm`: `krrcream | nk2 | nk3` (기본 `krrcream`; Krrcream은 원본 노트만 재배치하고 nK2는 키 수 확장 중 목표 레이아웃에 안전한 보조 노트를 직접 생성하며, NK3는 번들된 P64 hybrid ONNX와 host beam32로 재배치·보조 노트 생성을 함께 평가)
