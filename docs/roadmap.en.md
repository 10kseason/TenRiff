...
- Project version line is `1.2.102 stable`; public packages bundle no upscaler model. Selecting a compatible rights-cleared ONNX only stores its path, and BGA Upscaler remains off until the user enables it and accepts the high-spec warning; there is no automatic benchmark gate.
