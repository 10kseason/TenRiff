...
- Project version line is `1.4.4`; public packages bundle the deterministic NK3 conversion model, but no BGA upscaler model. Selecting a compatible rights-cleared upscaler ONNX only stores its path, and BGA Upscaler remains off until the user enables it and accepts the high-spec warning; there is no automatic benchmark gate.
