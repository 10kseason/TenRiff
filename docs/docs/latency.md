...
- **Stateful debounce**: Track a per-key state machine (UP/DOWN) so duplicate DOWNs while DOWN (and UPs while UP) are discarded, and collapse fast down→up→down chatter that arrives within ~5 ms.
