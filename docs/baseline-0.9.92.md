...
이 문서는 앞으로 TenRiff 작업을 이어갈 때 기준선으로 삼아야 하는 `0.9.92` 베이스 문서입니다. 목적은 "무엇을 현재 기본값으로 보고, 무엇을 유지해야 하며, 어떤 범위 안에서 다음 작업을 쌓아야 하는지"를 빠르게 고정하는 것입니다.
...
- 배포 기준 경로:
  - Windows 패키지: `Baepoks/TenRiff-0.9.92`
  - 공개 소스 패키지: `opensource-Tenriff-source/TenRiff-0.9.92-source`
...
- 다음 작업은 `0.9.92` 베이스를 깨지 않는 가산형 변경을 우선한다.
