...
- `gameplay/GameplayChart.*`
  - Converts BMS timelines into **sample-time-based note events**
...
- `gameplay/GameplayEngine.*`
  ...
  - **Hold tail rule**: only BMS `#LNMODE 2` charge notes evaluate release timing against the normal judgement window (head/tail 50:50)
...
- **Hold tail judgement** applies only to BMS `#LNMODE 2` charge notes; early release is treated as BAD
