...
이 문서는 앞으로 TenRiff 작업을 이어갈 때 기준선으로 삼아야 하는 `0.8.0` 베이스 문서입니다. 목적은 "무엇을 현재 기본값으로 보고, 무엇을 유지해야 하며, 어떤 범위 안에서 다음 작업을 쌓아야 하는지"를 빠르게 고정하는 것입니다.
...
- 배포 기준 경로:
  - Windows 패키지: `Baepoks/TenRiff-0.8.0`
  - 공개 소스 패키지: `opensource-Tenriff-source/TenRiff-0.8.0-source`
...
- BMS parser/normalizer/timeline은 실사용 호환 위주로 강화된 상태를 기준으로 본다.
...
- future note는 플레이필드 위 바깥에서 자연스럽게 들어오고, 노트 렌더는 플레이필드 안으로 클리핑되는 현재 연출을 기준선으로 삼는다.

## Baseline UX
- Song Select 좌측 버튼열은 `LEVEL / TITLE / SOURCES / KEY / BROWSE / MOD / RECORDS` 구조를 유지한다.
- Song Select의 `LEVEL`, `TITLE`, `KEY`는 메인 곡 선택 화면에서 바로 접근 가능한 빠른 필터/정렬 축이다.
- 좌측 버튼열과 주요 메뉴 버튼은 마우스 좌클릭 기준 즉시 선택/실행 가능해야 한다.
...
- 다음 작업은 `0.8.0` 베이스를 깨지 않는 가산형 변경을 우선한다.
