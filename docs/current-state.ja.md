...
現在のプロジェクト版は **1.7.7** です。後奏が終了すると追加入力なしで結果を保存・送信します。Stella、Satellite、U_E Pack 4K/6K/8Kの難易度表プリセットを追加しました。Sites連携と既存のオーディオ・スキン・セッション機能を維持し、既定の出力はWASAPIです。[1.7.7リリース案内](release-1.7.7-gate.md)に変更内容と検証範囲を記載しています。
...
- BMS key mode:
  ...
  - 1.7.7 公式 build/Windows ZIP は standalone BMS key-converter CLI/GUI を build・同梱しない。top-level CMake option は既定 `OFF` で、source は開発 regression 用のみ維持
...
- Direct-IP multiplayer:
  - protocol v6 は固定 TCP coordinator 1台に最大8人まで接続（Windows、既定 `27301/TCP`）
  ...
  - リーダーが共通Rateを0.50〜2.00倍（0.05単位）で選択し、変更時は全員のREADYを解除。判定・Gauge Shift・Random/Mods/Assist規則を維持し、各プレイヤーのローカルkey-mode conversionを許可
