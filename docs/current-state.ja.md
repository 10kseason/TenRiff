...
## Baseline
- 現在の stable release line は `1.4.5.2`
...
- BMS key mode:
  ...
  - 1.4.5.2 公式 build/Windows ZIP は standalone BMS key-converter CLI/GUI を build・同梱しない。top-level CMake option は既定 `OFF` で、source は開発 regression 用のみ維持
...
- 現在の stable P2P 配布ラインは `TenRiff 1.4.5.2`
