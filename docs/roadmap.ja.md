...
- プロジェクト版ラインは `1.2.8 stable`。公開 package は upscaler model を同梱しない。互換性と権利を確認済みの ONNX を選択しても path を保存するだけで、BGA Upscaler は user が有効化して high-spec warning を確認するまで off のまま。自動 benchmark gate はない。
