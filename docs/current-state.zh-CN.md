...
- BMS key mode：
  ...
  - `Conversion Note Add` 提供 `Default` / `Add 25%+`；后者仅在 playable key count 改变时，相对基础转换结果请求至少增加25%的安全无声和弦，写入 replay/result metadata，且不能替换普通最佳记录
...
## 运行时 / 打包规则
- 新用户 profile 会自动创建
- 当前正式 P2P 发布线为 `TenRiff 1.2.101 stable`
- 发布包不包含 `Songs`
