...
- `1.0.7` Windows 测试发布线为了隔离 RawInput 不稳定原因，已经把输入后端固定为 Polling
...
## 运行时 / 打包规则
- 新用户 profile 会自动创建
- 最近一次 staged 的发布包是 `Baepoks/TenRiff-1.0.7`
- 发布包不包含 `Songs`
- 发布包会同时包含用于菜单 BGM 的 `Mainmusic/` 运行时资源
- 发布更新时，只把已构建的产物放进 `Baepoks/`
- 如果请求 source-only/public handoff，用户偏好是先写 include/exclude 列表
- 最近一次 staged 的公开源代码包会像 `opensource-Tenriff-source/TenRiff-1.0.7-source` 这样按版本单独 staging
