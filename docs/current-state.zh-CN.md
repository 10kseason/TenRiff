...
- BMS key mode：
  ...
  - NK3 使用随包提供的 `models/NK3-P64-hybrid.onnx` 与 host beam32。默认使用 strict GPU；可通过 `TENRIFF_NK3_DEVICE=CPU` 选择 strict CPU。不支持 NPU 或自动 fallback，若未在指定设备执行则直接失败。
